...
# Simulated multi-node rank 1 on one physical machine.
# Start after node_0.sh; it joins the same Docker network/discovery file and uses GPUs 6,7.
...
HOST_GPUS="${NODE1_GPUS:-6,7}"
GPU_REQUEST="\"device=${HOST_GPUS}\""

NODE_NAME="${NODE_NAME:-stableaudio-node1}"
